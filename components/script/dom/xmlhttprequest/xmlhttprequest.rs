...
impl XMLHttpRequestMethods<crate::DomTypeHolder> for XMLHttpRequest {
    ...
    fn Send(
        ...
        let Some(url) = &*self.request_url.borrow() else {
            return Err(Error::InvalidState(None));
        };
        ...
        let global = self.global();
        let mut request =
            RequestBuilder::new(global.webview_id(), url.clone(), self.referrer.clone())
                    ...
                    for param in mime.params() {
                        if param.0 == CHARSET && !param.1.as_str().eq_ignore_ascii_case(encoding) {
                            let new_params: Vec<_> =
                                mime.params().filter(|p| p.0 != CHARSET).collect();
                            ...
                            if let Ok(content_type_header) = ContentType::from_str(&new_mime) {
                                request.headers.typed_insert(content_type_header);
                            }
                        }
                    }
                }
